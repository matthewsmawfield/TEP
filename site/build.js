...
async function buildStaticSite() {
    ...
    try {
        ...
        const sortedSections = manifest.sections.slice().sort((a, b) => a.order - b.order);
        ...
        for (const section of sortedSections) {
            ...
            if (fs.existsSync(componentPath)) {
                ...
                componentsHtml += `
                <section id="${section.id}" class="manuscript-section" data-section="${escapeAttr(section.title)}">
                    ${componentHtml}
                </section>`;
            } else {
        ...
        // Copy robots.txt, sitemap.xml, and other public root files to dist root
        ...
        // URL compatibility: this site was historically served from the repo
        // root, so icons, images, and the PDF were reachable at / rather than
        // /public/. Keep the old URLs working alongside the new layout.
        const publicDir = path.join(__dirname, 'public');
        const legacyRootFiles = fs.existsSync(publicDir)
            ? fs.readdirSync(publicDir).filter((f) =>
                /\.(ico|png|svg|jpe?g|webp)$/i.test(f) && fs.statSync(path.join(publicDir, f)).isFile())
            : [];
        ...
        if (fs.existsSync(docsDir)) {
            for (const file of fs.readdirSync(docsDir).filter((f) => f.endsWith('.pdf'))) {
                fs.copyFileSync(path.join(docsDir, file), path.join(distDir, file));
            }
        }
        if (legacyRootFiles.length) {
            console.log(`📁 Copied ${legacyRootFiles.length} legacy root asset(s) to dist root`);
        }
        ...
        const markdownFilename = markdownPath ? path.basename(markdownPath) : '(skipped)';
